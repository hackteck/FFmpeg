...
static int build_program(AVFilterContext *ctx)
{
    ...
    c->f_shader_source = (GLchar *)av_malloc(len * sizeof(*c->f_shader_source));
...
static int setup_gl(AVFilterLink *inLink)
{
    ...
    {
        av_log(ctx, AV_LOG_ERROR, "SDL initialization failed: %s\n", SDL_GetError());
        return -1;
    }

    c->window = SDL_CreateWindow("", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, inLink->w, inLink->h, SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN);
    if (!c->window)
    {
        av_log(ctx, AV_LOG_ERROR, "SDL window creation failed: %s\n", SDL_GetError());
        return -1;
    }

    c->glContext = SDL_GL_CreateContext(c->window);
    if (!c->glContext)
    {
        av_log(ctx, AV_LOG_ERROR, "SDL OpenGL context creation failed: %s\n", SDL_GetError());
        return -1;
    }
    ...
    {
        av_log(ctx, AV_LOG_ERROR, "Failed to build program\n");
        return -1;
    }
                                 ...
                                 const AVFrame *toFrame)
{
    ...
    SDL_GL_MakeCurrent(c->window, c->glContext);
...
static av_cold void uninit(AVFilterContext *ctx)
{
    ...
    if (c->glContext)
    {
        ...
        SDL_GL_DeleteContext(c->glContext);
    }
