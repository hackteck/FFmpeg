...
static int setup_gl(AVFilterLink *inLink)
{
    ...
    c->glContext = emscripten_webgl_create_context("canvas", NULL); // Initialize WebGL context
                                 ...
                                 const AVFrame *toFrame)
{
    ...
    emscripten_webgl_make_context_current(c->glContext);
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    emscripten_set_main_loop_arg((em_arg_callback_func)blend_frame, &c->fs, 0, 1);
...
static av_cold void uninit(AVFilterContext *ctx)
{
    ...
    emscripten_webgl_destroy_context(c->glContext);
}
